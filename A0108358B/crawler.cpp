/**
Author: A0108358B, Nicholas Lum Aik Yong

Crawler mechanism inspired by https://gist.githubusercontent.com/berlinbrown/4583728/raw/8cb21d43e2dc80ce28dec20ea8595f8430ff7c87/gistfile1.cpp
*/
#include <stdio.h>
#include <string>
#include <iostream>
...
int writeUrls(vector<Url*> &urls, string filename) {
    ofstream urlsfile;
    urlsfile.exceptions ( ofstream::eofbit | ofstream::failbit | ofstream::badbit );
    try {
        ...
        cout << "IO error when writing to " << filename << endl;
        return 1;
    }
    urlsfile.close();
}
...
int main() {
    ...
    for (int i = 0;
        ...
        future<vector<Url*>> f1 = async(crawl, url);
        
        // second thread
        cout << "Crawling " << url2->host << url2->path << endl;
        future<vector<Url*>> f2 = async(crawl, url2);
        
        vector<Url*> new_urls = f1.get();
        if (!url->accessible) {
        ...
        vector<Url*> new_urls2 = f2.get();
        if (!url2->accessible) {
            cout << "Cannot connect to: " << url2->get() << endl;
        } else {
            add_to(urls, new_urls2);
            crawled++;
        }
